...
针对 DeepSeek Harness `@deepseek-ai/dsh@0.1.0-rc.7`（Cordis `apply` + `ctx.tools.register` 仍要求 `output.render`）。`engines.dsh` 声明 `>=0.1.0-rc.7 <0.2.0`。rc.7 新增的插件设置卡片（`installSettingsSection` / `dsh.client`）是可选项，本管理器暂不需要。
## 许可证
